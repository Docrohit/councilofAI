...
const imported = importedRaw
  ...
  : undefined;
export const benchmarkSuite = imported?.tasks || smokeSuite;
    ...
    try {
      for (let repeat = 1; repeat <= result.repeats; repeat++)
        for (const id of result.taskIds) {
          ...
          const task = benchmarkSuite.find((t) => t.id === id)!;
          const prompt =
            task.prompt +
      ...
      result.summary = {
        ...
        councilAccuracy: correct("council") / count,
        baselineAccuracy: correct("baseline") / count,
